...
//
// Purpose: Milso, recon pistol. Base copied from weapon_pistol.cpp
//
//=============================================================================//
...
#define	MILSO_FASTEST_REFIRE_TIME		0.1f
#define	MILSO_FASTEST_DRY_REFIRE_TIME	0.2f

#define	MILSO_ACCURACY_SHOT_PENALTY_TIME		0.2f	// Applied amount of time each shot adds to the time we must recover from
...
class CWeaponMilso : public CBaseHL2MPCombatWeapon
{
public:
	DECLARE_CLASS( CWeaponMilso, CBaseHL2MPCombatWeapon );
...
void CWeaponMilso::UpdatePenaltyTime( void )
{
	...
	if ( ( ( pOwner->m_nButtons & IN_ATTACK ) == false ) && ( m_flSoonestPrimaryAttack < gpGlobals->curtime ) )
	{
		...
		m_flAccuracyPenalty = clamp( m_flAccuracyPenalty, 0.0f, MILSO_ACCURACY_MAXIMUM_PENALTY_TIME );
	}
}
...
void CWeaponMilso::ItemPostFrame( void )
{
	...
	{
		m_flLastAttackTime = gpGlobals->curtime + MILSO_FASTEST_REFIRE_TIME;
		m_flSoonestPrimaryAttack = gpGlobals->curtime + MILSO_FASTEST_REFIRE_TIME;
		m_flNextPrimaryAttack = gpGlobals->curtime + MILSO_FASTEST_REFIRE_TIME;
	}
...
void CWeaponMilso::AddViewKick( void )
{
	...
	viewPunch.y = SharedRandomFloat( "milsopay", -.6f, .6f );
	viewPunch.z = 0.0f;
